...
class DetectionManager:
    ...
    def __init__(self):
        ...
        self.SAVE_RESOLUTION_ZOOMED = 512  # Resolution for zoomed images, may be used for reclassification.
        ...
        self.detector_lock = threading.Lock()   # Protects detector instance during reinitialization.
    ...
    def _detection_loop(self):
        ...
        while not self.stop_event.is_set():
            ...
            try:  
                ...
                if self.previous_frame_hash is not None and current_frame_hash == self.previous_frame_hash:
                    self.consecutive_identical_frames += 1
                    logger.warning(f"Identical frame detected. Consecutive count: {self.consecutive_identical_frames}.")
            ...
            if object_detected:
                ...
                if detection_info_list:
                    try:
                        ...
                        zoomed_frame_raw = self.create_square_crop(original_frame, bbox, margin_percent=0.1)

                        if zoomed_frame_raw is not None and zoomed_frame_raw.size > 0:
                            zoomed_frame = cv2.resize(zoomed_frame_raw,
                                                      (self.SAVE_RESOLUTION_ZOOMED, self.SAVE_RESOLUTION_ZOOMED))
