...
def extract_original_exif(filepath: str) -> bytes | None:
    """
...
def ingest_folder(folder_path: str, source_id: int, move_files: bool = False):
    ...
    """
    logger.info(
        f"Starting ingest from: {folder_path} (Source ID: {source_id}, Move: {move_files})"
    )
...
def _handle_file_move(root: str, filename: str, status: str):
    ...
    if status == "ingested":
        target_dir = os.path.join(
            config.get("OUTPUT_DIR", root), "processed", timestamp_folder
        )
    elif status == "skipped":
    ...
    if object_detected and detection_info_list:
        ...
        if image.shape[1] > 800:
            optimized_frame = cv2.resize(
                image, (800, int(image.shape[0] * 800 / image.shape[1]))
            )
            cv2.imwrite(
                optimized_path, optimized_frame, [int(cv2.IMWRITE_WEBP_QUALITY), 80]
            )
        else:
            ...
            try:
                ...
                if sq_x2 > sq_x1 and sq_y2 > sq_y1:
                    ...
                    if square_crop.size > 0:
                        ...
                        cv2.imwrite(
                            thumb_path, thumb_img, [int(cv2.IMWRITE_WEBP_QUALITY), 80]
                        )
                else:
                    thumb_filename = None
            except Exception as e:
...
def ingest_inbox_folder(pending_dir: str, file_snapshot: list[str]):
    ...
    - inbox/error/ for files that failed to process

    Args:
        pending_dir: Path to inbox/pending directory
        file_snapshot: List of absolute file paths to process (snapshot taken at start)
    """
    import shutil
    ...
    logger.info(
        ...
        f"Ingested: {count_ingested}, Skipped: {count_skipped}, Errors: {count_errors}"
    )
