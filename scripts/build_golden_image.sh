...
# Scheduled backup service + timer.
...
# Guided recovery runner + narrowly scoped start permission.
sudo cp "$repo_root/rpi/systemd/wmb-recovery.service" "$mount_point/tmp/systemd/"
sudo cp "$repo_root/rpi/polkit/10-watchmybirds-recovery.rules" "$mount_point/tmp/polkit/"
